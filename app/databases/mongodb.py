...
async def upsert_user(user_id, username=None, phone=None, role="user", name=None):
    ...
    if not user:
        ...
        # Update role for boss and if explicitly passed
...
async def get_bosses():
    return await users_col.find({"role": "boss"}).to_list(None)
...
async def get_apartments(only_available=False):
    ...
    if only_available:
        ...
        q = {
            "is_available": True,
            "availableFromMonth": {"$lte": current_month},
            "availableToMonth": {"$gte": current_month}
        }
...
async def create_booking(user_id, ap_id, start_date, end_date, phone, wishes, total_price):
    res = await bookings_col.insert_one({
        "user_id": user_id, "ap_id": ObjectId(ap_id), "start_date": start_date, "end_date": end_date,
        "phone": phone, "wishes": wishes, "total_price": total_price,
        "prepayment": total_price * 0.5, "remaining": total_price * 0.5, "status": "pending_50", "created_at": datetime.datetime.utcnow()
    })
